...
Rpc._parseTxResult = function(info) {
  ...
  var n =0;
   ...
   if (info.vin.length == 1 && info.vin[0].coinbase && info.vin[0].sequence > 0)
    info.isCoinStake = true;
  
  // -- coinstake is marked by empty vout 0
  if (info.vout.length > 1 && info.vout[0].value == 0)
  {
    info.isCoinStakeOut = true;
